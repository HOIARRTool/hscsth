...
st.sidebar.caption(
    "อัปโหลดไฟล์ Excel 2 ปีสำหรับใช้งานใน session นี้ โดยไม่ต้องเก็บไฟล์ไว้ใน GitHub repo หรือ redeploy Render"
)
...
for year, cfg in HSCS_YEAR_CONFIG.items():
    uploaded_file = st.sidebar.file_uploader(
        ...
        help=f"คาดหวังโครงสร้างชีตแบบเดิม และ prefer ชีตชื่อ {cfg['default_sheet']}",
    )
...
if not uploaded_year_data:
    ...
    st.markdown(
        ...
        **รูปแบบไฟล์ที่คาดหวัง**
        ...
        - Row 3: หน่วยงาน
        - Column A: มิติหลัก
        - Column B: มิติย่อย เช่น `A1. ...`
        - Column C onward: คะแนน % Positive Response
